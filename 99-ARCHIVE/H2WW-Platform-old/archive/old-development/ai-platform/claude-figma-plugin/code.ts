...
class DesignCreator {
  private createdNodes: SceneNode[] = [];
  public tokenResolver = new TokenResolver();
  
  async createFromSpec(spec: any, tokens?: any): Promise<void> {
    try {
  ...
  public async createElement(spec: any): Promise<SceneNode | null> {
    if (!spec || !spec.type) {
  ...
  private async createFrame(spec: any): Promise<FrameNode> {
  ...
  private async createText(spec: any): Promise<TextNode> {
  ...
  private async createRectangle(spec: any): Promise<RectangleNode | FrameNode> {
  ...
  private async createEllipse(spec: any): Promise<EllipseNode> {
    const ellipse = figma.createEllipse();
    
    ellipse.name = spec.name || 'Ellipse';
    ellipse.resize(spec.width || 100, spec.height || 100);
    
    if (spec.x !== undefined) ellipse.x = spec.x;
    if (spec.y !== undefined) ellipse.y = spec.y;
  ...
  private async createComponent(spec: any): Promise<ComponentNode> {
    const component = figma.createComponent();
    
    component.name = spec.name || 'Component';
    component.resize(spec.width || 200, spec.height || 100);
    
    if (spec.x !== undefined) component.x = spec.x;
    if (spec.y !== undefined) component.y = spec.y;
  ...
  private async createGroup(spec: any): Promise<GroupNode> {
    ...
    if (nodes.length > 0) {
      ...
      if (spec.x !== undefined) group.x = spec.x;
      if (spec.y !== undefined) group.y = spec.y;
  ...
  private hexToRgbWithAlpha(input: string): RGBA {
    // Handle rgba() format
  ...
  private getFontStyle(weight?: number | string): string {
...
// Import design tokens as Figma variables
async function importTokensAsVariables(tokens: any): Promise<void> {
  try {
...
async function createColorVariables(colorTokens: any, collectionName: string): Promise<void> {
  const collection = figma.variables.createVariableCollection(collectionName);
  
  function processColorObject(obj: any, prefix: string = ''): void {
    for (const [key, value] of Object.entries(obj)) {
      if (value && typeof value === 'object') {
        if ((value as any).$value && typeof (value as any).$value === 'string') {
          // This is a token with a value
          const variable = figma.variables.createVariable(`${prefix}${key}`, collection, 'COLOR');
          const rgb = hexToRgb((value as any).$value as string);
...
async function createDimensionVariables(spacingTokens: any, collectionName: string): Promise<void> {
  const collection = figma.variables.createVariableCollection(collectionName);
  
  function processSpacingObject(obj: any, prefix: string = ''): void {
    for (const [key, value] of Object.entries(obj)) {
      if (value && typeof value === 'object') {
        if ((value as any).$value) {
          const variable = figma.variables.createVariable(`${prefix}${key}`, collection, 'FLOAT');
          const numValue = parseInt((value as any).$value.toString().replace('px', ''));
...
async function createStringVariables(typographyTokens: any, collectionName: string): Promise<void> {
  const collection = figma.variables.createVariableCollection(collectionName);
  ...
  for (const [key, value] of Object.entries(typographyTokens)) {
    if (value && typeof value === 'object' && (value as any).$value) {
      ...
      variable.setValueForMode(collection.defaultModeId, (value as any).$value);
    }
  }
}

// Import component library
async function importComponentLibrary(components: any, tokens: any): Promise<void> {
...
async function importCompleteSystem(tokens: any, components: any): Promise<void> {
