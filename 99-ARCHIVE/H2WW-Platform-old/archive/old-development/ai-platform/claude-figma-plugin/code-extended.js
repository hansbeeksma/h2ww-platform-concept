// H2WW Complete Design System Importer - Extended JavaScript Version
console.log('H2WW Complete Design System Plugin starting...');

// Show UI with larger size for all components
...
// Token resolver for H2WW system
class TokenResolver {
  constructor() {
    this.tokens = {};
  }
  ...
  resolveDimension(dimensionToken) {
    ...
    if (typeof resolved === 'string') {
      return parseInt(resolved.replace(/[^0-9]/g, '')) || 16;
    }
...
class ComponentFactory {
  ...
  async createColorPalette(system) {
    ...
    if (system.visualLanguage && system.visualLanguage.color && system.visualLanguage.color.palettes) {
      for (const [category, colors] of Object.entries(system.visualLanguage.color.palettes)) {
        const section = await this.createColorSection(category, colors);
  ...
  async createColorSection(name, colors) {
    ...
    for (const [colorName, colorValue] of Object.entries(colors)) {
      if (typeof colorValue === 'object' && colorValue !== null) {
        const swatch = this.createColorSwatch(
          colorName,
          colorValue.hex || colorValue.$value || '#000000'
        );
  ...
  // Typography System
  async createTypographySystem(system) {
    const frame = figma.createFrame();
    ...
    if (system.visualLanguage && system.visualLanguage.typography && system.visualLanguage.typography.hierarchy) {
      for (const [level, styles] of Object.entries(system.visualLanguage.typography.hierarchy)) {
        ...
        text.fontSize = parseInt(styles.size) || 16;
        text.fontName = {
          family: "Inter",
          style: parseInt(styles.weight) >= 600 ? "Bold" : "Regular"
        };
  ...
  async createFormField(type, spec) {
  ...
  // Patterns
  async createAuthenticationForm(spec) {
...
class H2WWDesignSystemCreator {
  ...
  async createCompleteSystem(systemData) {
    try {
      console.log('Creating complete H2WW Design System...');
      ...
      console.log('System creation completed successfully');

    } catch (error) {
      console.error('Error creating design system:', error);
      figma.notify('❌ Error creating design system: ' + error.message, { timeout: 3000 });
    }
  }
}
  ...
  try {
    if (msg.type === 'create-complete-system') {
      ...
      switch (msg.componentType) {
        ...
        case 'text-input':
        case 'email-input':
          component = await factory.createFormField(msg.componentType.replace('-input', ''), msg.spec);
          break;
        case 'authentication':
        ...
        case 'dashboard-page':
          ...
          break;
        case 'color-palette':
          component = await factory.createColorPalette(msg.tokens || {});
          break;
        case 'typography':
          component = await factory.createTypographySystem(msg.tokens || {});
          break;
        default:
          figma.notify(`Component type "${msg.componentType}" not implemented yet`, { timeout: 2000 });
          return;
      }
    ...
    figma.notify('❌ Error: ' + error.message, { timeout: 3000 });
  }
};

// Initialize plugin
console.log('H2WW Complete Design System Plugin ready!');
